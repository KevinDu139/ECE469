...
void main (int argc, char *argv[])
{
  ...
  lock_t buffer_lock;             // Lock used by child processes for accessing buffer
  char h_mem_str[10];             // Used as command-line argument to pass mem_handle to new processes
  char s_procs_completed_str[10]; // Used as command-line argument to pass page_mapped handle to new processes
  char buffer_lock_str[10];       // Used as command-line argument
...
Printf("got to here 1\n");
  ...
  // Create lock for use by other processes
  ...
  for(i=0; i<numprocs; i++) {
    ...
    // Create Producer
    process_create(FILENAME_PRODUCER, h_mem_str, s_procs_completed_str, buffer_lock_str);
    // Create Consumer
    process_create(FILENAME_CONSUMER, h_mem_str, s_procs_completed_str, buffer_lock_str);
