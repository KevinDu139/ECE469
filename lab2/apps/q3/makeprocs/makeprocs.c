...
void main (int argc, char *argv[])
{
	...
	buffer_l *ring_buffer;
	uint32 h_mem;                   // Used to hold handle to shared memory page
	sem_t s_procs_completed;        // Semaphore used to wait until all spawned processes have completed
	sem_t s_fullslots;				//semaphore used to hold number of full slots available
	sem_t s_emptyslots; 			//semaphore used to hold number of empty slots available
	lock_t buffer_lock;             // lock for program
  ...
  for(i=0; i<numprocs; i++) {
    ...
    // Create Producer
    process_create(FILENAME_PRODUCER, h_mem_str, s_procs_completed_str, buffer_lock_str, s_fullslots_str, s_emptyslots_str);
    // Create Consumer
    process_create(FILENAME_CONSUMER, h_mem_str, s_procs_completed_str, buffer_lock_str, s_fullslots_str, s_emptyslots_str);
