...
//
//----------------------------------------------------------------------
void ProcessSchedule (unsigned suspendFlag) {
  PCB *pcb=NULL;
  int i=0;
  Link *l=NULL;
  ...
  //if a process is suspended, it is removed from the run queue by ProcessSuspend. Therefore,
  //the top of run queue has the process to be scheduled and moving top of run queue is not 
  //needed in this case.
  if (!suspendFlag) {
...
void main (int argc, char *argv[])
{
  ...
  if (userprog != (char *)0) {
    ...
    printf("calling process create with %d parameters\n", numargs); 
    switch(numargs) {
      ...
      case  3: printf("calling process_create with three parameters\n");
	       process_create(params[0], params[1], params[2], NULL); break;
